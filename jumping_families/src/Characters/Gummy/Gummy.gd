...
func updateGraphics(__velocity):
	
	if is_on_floor():
		if abs(__velocity.x) <= MIN_VELOCITY_X_TO_FLIP_GRAPHICS:
			graphics.frame = IDLE_ANIMATION_FRAME
			animationPlayer.stop()
		else:
			animationPlayer.play("Walk")
	if __velocity.x > MIN_VELOCITY_X_TO_FLIP_GRAPHICS:
		graphics.flip_h = false
	elif __velocity.x < -MIN_VELOCITY_X_TO_FLIP_GRAPHICS:
